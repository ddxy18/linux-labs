...
static int romfs_readpage(struct file *file, struct page *page) {
  ...
  if (offset < size) {
    ...
    if (strncmp(encrypted_file_name, file->f_path.dentry->d_name.name,
                ROMFS_MAXFN) == 0) {
      int i = 0;
      for (; i < fillsize; i++) {
...
static const struct address_space_operations romfs_aops = {.readpage =
                                                               romfs_readpage};
...
static int romfs_readdir(struct file *file, struct dir_context *ctx) {
  ...
  for (;;) {
    ...
    if (strncmp(fsname, hided_file_name, ROMFS_MAXFN))
...
static struct dentry *romfs_lookup(struct inode *dir, struct dentry *dentry,
  ...
  if (strncmp(exec_file_name, name, ROMFS_MAXFN) == 0) {
...
static struct inode *romfs_iget(struct super_block *sb, unsigned long pos) {
  ...
  default:
    ...
    init_special_inode(i, mode, MKDEV(nextfh >> 16, nextfh & 0xffff));
    break;
  }
...
static int __init init_romfs_fs(void) {
  ...
  romfs_inode_cachep = kmem_cache_create(
      "romfs_i", sizeof(struct romfs_inode_info), 0,
      SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD | SLAB_ACCOUNT, romfs_i_init_once);
